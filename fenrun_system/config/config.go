...
var ossConn *viper.Viper
var upLoad *viper.Viper
...
func init() {
	...
	//OSS
	ossConn = viper.Sub("settings.oss")
	if ossConn == nil {
		panic("config not found settings.oss")
	}
	OSSConfig = InitOSSDatabase(ossConn)

	//UPLOADFILE
	upLoad = viper.Sub("settings.upload")
	if upLoad == nil {
		panic("config not found settings.upload")
	}
	UpdateFile = InItUpLoadFile(upLoad)
