...
import (
	...
	"xAdmin/models"
	"xAdmin/pkg"
	"xAdmin/pkg/googleAuthenticator"
	"xAdmin/utils"
...
func GetUserReferrer(c *gin.Context) {
	var data models.SysUser
	//var res models.Response
	...
	result, err := data.GetUserReferrer(param)
	//if err !=nil{
	//	res.Msg = err.Error()
...
func GetUserReferrals(c *gin.Context) {
	...
	result, err := data.GetUserReferrals(param)
	//if err !=nil{
	//	res.Msg = err.Error()
	//	c.JSON(http.StatusOK, res.ReturnError(0))
	//}
...
func GetUserPerformance(c *gin.Context) {
	...
	err := per.GetVipTotal()
	pkg.AssertErr(err, "获取业绩总数失败", -1)
	err = per.GetTotal()
	pkg.AssertErr(err, "获取真实总业绩失败", -1)
	err = per.GetToday()
...
func GetUserAuditRecord(c *gin.Context) {
	pageSizeStr := c.Request.FormValue("pageSize")
	pageIndexStr := c.Request.FormValue("pageIndex")
	pageSize, err := strconv.ParseInt(pageSizeStr, 10, 64)
	pkg.AssertErr(err, "pageSize错误", -1)
	...
	userAudit := models.NewUserAudit()
	curUserID := utils.GetUserIdStr(c)
	role := utils.GetRolekey(c)

	list, total, err := userAudit.GetAuditList(pageIndex, pageSize, curUserID, role)
	var res models.Response
	if err != nil {
		res.Msg = err.Error()
		c.JSON(http.StatusOK, res.ReturnError(400))
...
func SubmitNewUser(c *gin.Context) {
	...
	var res models.Response
	if err = sysUser.SubmitNewUser(submitterID); err != nil {
		res.Msg = err.Error()
		c.JSON(http.StatusOK, res.ReturnError(400))
		return
	}
...
func Editlevellock(c *gin.Context) {
	var sysUser models.SysUser
	param := make(map[string]string)
	param["id"] = c.Request.FormValue("userid")
	param["level_lock"] = c.Request.FormValue("level_lock")
	nick_name := c.Request.FormValue("nick_name")
	var res models.Response
	err := sysUser.Editlevellock(param)
	if err != nil {
		res.Code = 0
		res.Msg = err.Error()
	}
	...
	loginlog.Msg = "打开 " + nick_name + " 的等级锁定"
	if param["level_lock"] == "0" {
		loginlog.Msg = "取消 " + nick_name + " 的等级锁定"
	}
	loginlog.Create()
	c.JSON(http.StatusOK, res.ReturnOK())
}
