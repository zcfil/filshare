...
func Mysqlconn(typesql string) (conns string, dbType string) {
	...
	switch typesql {
	case "center":
		dbType = config2.CenterDatabaseConfig.Dbtype
		host = config2.CenterDatabaseConfig.Host
		port = config2.CenterDatabaseConfig.Port
		database = config2.CenterDatabaseConfig.Database
		username = config2.CenterDatabaseConfig.Username
		password = config2.CenterDatabaseConfig.Password
	case "admin":
