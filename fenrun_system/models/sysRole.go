...
func (role *SysRole) GetList() (SysRole []SysRole, err error) {
	...
	if role.RoleKey != "admin" {
		table = table.Where("role_key <> ?", "admin")
	}
	if role.RoleKey == "common" {
...
func (role *SysRole) GetRoleMeunId() ([]int64, error) {
	...
	if err := orm.Eloquent.Table("sys_role_menu").Select("menu_id").Joins("LEFT JOIN sys_menu on sys_menu.menuId=sys_role_menu.menu_id").Where("role_id = ? ", role.Id).Where(" menu_id not in(select distinct sys_menu.parentId from sys_role_menu LEFT JOIN sys_menu on sys_menu.menuId=sys_role_menu.menu_id where role_id =? and sys_menu.parentId is not null )", role.Id).Find(&menuList).Error; err != nil {
		return nil, err
	}
...
func (role *SysRole) GetRoleMeunIdNew() ([]int64, error) {
	menuIds := make([]int64, 0)
	menuList := make([]MenuIdList, 0)
	if err := orm.Eloquent.Table("sys_role_menu").Select("menu_id").Joins("LEFT JOIN sys_menu on sys_menu.menuId=sys_role_menu.menu_id").Where("role_id = ? ", role.Id).Where(" menu_id not in(select distinct sys_menu.parentId from sys_role_menu LEFT JOIN sys_menu on sys_menu.menuId=sys_role_menu.menu_id where role_id =? and sys_menu.parentId is not null )", role.Id).Find(&menuList).Error; err != nil {
		return nil, err
...
func (e *SysRole) BatchDelete(id []int64) (Result bool, err error) {
	...
	if err = orm.Eloquent.Table("sys_role").Where("is_del=0 and role_key != 'admin' and id in (?)", id).Update(mp).Error; err != nil {
		return
	}
	Result = true
	return
