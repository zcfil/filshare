...
type SysOperLog struct {
	...
	//操作时间
	OperTime time.Time `json:"operTime" gorm:"column:operTime;"`
	//返回数据
...
func (e *SysOperLog) BatchDelete(id []int64) (Result bool, err error) {
	if err = orm.Eloquent.Table("sys_operlog").Where("is_del=0 and operId in (?)", id).Update(map[string]interface{}{"is_del": "1", "update_time": time.Now(), "update_by": e.UpdateBy}).Error; err != nil {
		return
	}
	Result = true
	return
