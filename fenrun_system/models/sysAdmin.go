...
func (e *SysUser) GetPage(pageSize int, pageIndex int) ([]SysAdminPage, int32, error) {
	...
	// if e.NickName != "" {
	// 	table = table.Where("nick_name = ?", e.NickName)
	// }
	if e.Phone != "" {
...
func (e *SysUser) Editlevellock(param map[string]string) (err error) {
	sql := ` update sys_user set level_lock = ` + param["level_lock"] + ` where user_id = ` + param["id"]
...
func (e *SysUser) GetUserReferrals(param map[string]string) (interface{}, error) {
	...
	//sql := `select user_id,username,nick_name,phone,ifnull(accumulative,0)+ifnull(diff_value,0) as performance from sys_user where user_id in(`+als+`) order by field(user_id,`+als+`) `
	sql := `select user_id,username,nick_name,phone,set_level,lifts,accumulative,vip,ifnull(accumulative+real_value,0) as performance from sys_user where user_id in(` + als + `) order by field(user_id,` + als + `) `
	param["total"] = GetTotalCount(sql)
	//分页 and 排序
	//param["sort"] = "user_id"
	//param["order"] = "desc"
...
func (e *SysUser) AminId(adminusername string) (ret int, err error) {
	sql := `SELECT  user_id   FROM sys_user WHERE username = '` + adminusername + `'`
	type find struct {
...
func (e *SysUser) Getadmindiff(username string) (flag bool, err error) {
	flag = false
	sqlx := `SELECT  role_id  FROM  sys_user WHERE  username= "%s" AND  role_id = 5 `
	sqlx = fmt.Sprintf(sqlx, username)
	if err = orm.Eloquent.Debug().Raw(sqlx).Error; err != nil {
		log.Error(" 此用户不存在 : ", err.Error())
		return
	}
