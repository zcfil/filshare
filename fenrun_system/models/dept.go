...
type DeptInfo struct {
	Dept
	LeaderName string `json:"leader" gorm:"column:leader_name"` // 负责人名字, 不需要在部门表保存
}
...
func (e *Dept) GetDeptInfo() (*DeptInfo, error) {
	var info DeptInfo
	sql := `select d.*, u.nick_name leader_name from sys_dept as d left join sys_user as u on u.user_id = d.leader_id 
	where (d.deptId = ` + strconv.FormatInt(e.Deptid, 10) + `) and (d.is_del = 0)  
	order by d.deptId asc limit 1`

	if err := orm.Eloquent.Raw(sql).Scan(&info).Error; err != nil {
		return nil, err
	}
	return &info, nil
}
