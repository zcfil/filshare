...
type FuncWeek func(param map[string]string) (interface{}, error)               //定义函数结构
...
func GetExcelSummary(fun FuncTotal, param map[string]string) (string, error) {
	...
	filename = "客户姓名,业务员姓名,销售," + re["keys"].(string) + ",业务部门,业务员,备注,投资时间"
	//数据转为[]map
...
func GetWeekCustomerList(fun FuncWeek, param map[string]string) (string, error) {
	//获取参数
	sheetstr := param["sheet"]
	filefield := ""
	filename := ""
	...
	filefield = "name,phone,to_balance,lock_release,amount"
	filename = "用户名,手机号,直接释放收益,线性释放收益,总收益"
